...
from app.use_cases.show_fetching.models import (
    DatesFoundProgress,
    FetchedShowItem,
...
from app.use_cases.show_fetching.result_builder import (
    CinemaShowData,
    FinalMovieShowData,
    MovieShowData,
...
class ShowForSelectedMovieFetcher:
    ...
    def __init__(
        self,
        gateway: ShowFetchingGateway | None = None,
        builder: MovieShowDataBuilder | None = None,
    ) -> None:
        self.gateway = gateway or ShowFetchingGateway()
    ...
    async def fetch_shows_for_selected_movies(
        ...
        tasks = [
            self._process_single_movie(movie_id, city_id, progress_callback)
    ...
    async def _process_single_date(
        ...
        try:
            ...
            valid_results: list[list[FetchedShowItem]] = []
            for cinema_result in cinema_results:
                if isinstance(cinema_result, BaseException):
                    if self._is_degradable_error(cinema_result):
                        ...
                        continue
                    raise cinema_result
                valid_results.append(cinema_result)
