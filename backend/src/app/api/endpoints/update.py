...
from app.services.update.result_builder import UpdateCinemaResult, UpdateMovieResult
    ...
    async def run_update() -> None:
        try:
            result = await update_service.update_movie(
                ...
                progress_callback=_make_progress_pusher(event_queue),
            )
            await event_queue.put({"type": "complete", "data": build_update_movie_data(result).model_dump()})
        except Exception as error:
            await event_queue.put({"type": "error", "error": map_update_stream_error(error)})
        finally:
...
async def encode_update_cinema_stream(city_id: int, force_update_all: bool) -> AsyncIterator[str]:
    ...
    async def run_update() -> None:
        try:
            ...
            await event_queue.put({"type": "complete", "data": build_update_cinema_data(result).model_dump()})
        except Exception as error:
            await event_queue.put({"type": "error", "error": map_update_stream_error(error)})
        finally:
            await event_queue.put(None)
