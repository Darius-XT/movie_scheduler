    ...
    def get_movies_without_details(self) -> List[Movie]:
        ...
        Returns:
            List[Movie]: 缺少详细信息的电影列表，每个元素是 Movie 对象。
                缺少以下任一字段的电影会被选中：director、country、release_year
                如果所有电影都有详细信息，返回空列表 []。
                示例返回值: [Movie(id=123456, title="新电影", director=None, country=None, release_year=None, ...)]
        """
        try:
            return (
                ...
                .filter(
                    (
                        ...
                        | ((Movie.release_year.is_(None)) | (Movie.release_year == ""))
                    )
                )
                .all()
            )
