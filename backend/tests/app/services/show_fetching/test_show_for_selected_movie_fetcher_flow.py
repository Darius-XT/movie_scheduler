...
from app.services.show_fetching.fetcher import ShowForSelectedMovieFetcher
from app.services.show_fetching.entities import FetchedShowItem, ShowFetchProgressEvent
from app.services.show_fetching.result_builder import MovieShowDataBuilder
from app.services.show_fetching.gateway import ShowFetchingGateway


class FakeShowFetchingGateway:
    """用于测试的场次抓取网关。"""
...
def test_fetcher_builds_complete_movie_result() -> None:
    ...
    fetcher = ShowForSelectedMovieFetcher(
        gateway=cast(ShowFetchingGateway, FakeShowFetchingGateway()),
...
def test_fetcher_skips_movie_when_degradable_error_occurs() -> None:
    ...
    class PartiallyFailingGateway(FakeShowFetchingGateway):
        def get_show_dates(self, movie_id: int, city_id: int) -> list[str]:
            if movie_id == 2:
    ...
    fetcher = ShowForSelectedMovieFetcher(
        gateway=cast(ShowFetchingGateway, PartiallyFailingGateway()),
        builder=MovieShowDataBuilder(),
    )
...
def test_fetcher_raises_when_critical_error_occurs() -> None:
    ...
    class CriticallyFailingGateway(FakeShowFetchingGateway):
        def get_movie_name(self, movie_id: int) -> str:
            if movie_id == 2:
    ...
    fetcher = ShowForSelectedMovieFetcher(
        gateway=cast(ShowFetchingGateway, CriticallyFailingGateway()),
        builder=MovieShowDataBuilder(),
    )
