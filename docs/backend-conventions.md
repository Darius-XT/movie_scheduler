...
有状态对象（repository、service、client）在模块底部实例化为单例，通过 import 注入。测试用 `monkeypatch` 替换。纯函数工具类不需要单例：
...
| 异常 | 含义 | 处理方式 |
|------|------|----------|
| `ExternalDependencyError` | 外部 HTTP 失败 | 可降级，跳过当前条目继续 |
| `DataParsingError` | 解析结果为空 | 可降级，跳过当前条目继续 |
| `RepositoryError` | 数据库操作失败 | 向上传播，终止流程 |
...
状态码：`200` 成功 / `400` 客户端错误 / `404` 不存在 / `500` 服务器错误（不暴露细节）
