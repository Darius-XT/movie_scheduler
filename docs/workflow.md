...
- 测试目录镜像 `src/movie_scheduler/`,文件命名 `test_{被测模块名}.py`
...
发现 Pyright 报错时,优先按真实类型问题修复代码;如果是 SQLAlchemy ORM 动态字段等静态分析无法推断的场景,只能使用项目允许的 `getattr()` + `cast()` 方式局部收敛类型,不能用 `cast()` 掩盖普通类型错误。
