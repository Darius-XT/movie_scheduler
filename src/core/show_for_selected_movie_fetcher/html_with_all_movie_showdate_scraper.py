...
class HtmlWithAllMovieShowDateScraper:
    def __init__(self):
        self.base_url = "https://www.maoyan.com/cinemas?movieId={movie_id}"
        self.headers = DEFAULT_HEADERS.copy()
        self.session = requests.Session()
        self._warmed_up = False
        self._current_city_id = None

    def scrape_html_with_all_movie_showdate(
        self, movie_id: int, city_id: int | None = None
    ) -> tuple[bool, str]:
        """爬取指定电影的放映日期信息页面的HTML内容
        ...
        try:
            ...
            if not self._warmed_up:
                self._warm_up_session(city_id)
                self._warmed_up = True

            url = self.base_url.format(movie_id=movie_id)
            logger.debug(f"开始爬取放映日期信息: movieId={movie_id}, url={url}")
            ...
            if response.status_code == 200:
                logger.debug("成功获取放映日期信息HTML内容")
                return True, response.text
            else:
                logger.warning(f"请求失败，状态码: {response.status_code}")
                return False, ""
        except Exception as e:
            logger.error(f"获取放映日期信息失败: {e}")
            return False, ""
    ...
    def _warm_up_session(self, city_id: int):
        try:
            warmup_url = "https://www.maoyan.com/"
...
html_with_all_movie_showdate_scraper = HtmlWithAllMovieShowDateScraper()
