...
class Scraper:
    def __init__(self):
        ...
        self.headers = {
            ...
            "Cookie": self.cookies,  # 添加cookies到请求头
            "Host": "www.maoyan.com",
    ...
    # 设置城市: 更新 cookie 并重新设置 header 与 session
    def set_city(self, city: str):
        # 根据城市更新 cookies, 并据此更新 header
    ...
    def reset_cookies_by_city(self, city: str):
        ...
        recent_cis = f"{city_id}%3D10%3D1245%3D1126%3D150"

        # 解析现有的Cookie字符串并更新相关值
        cookie_parts = self.cookies.split("; ")
        updated_cookies = []

        for part in cookie_parts:
            if part.startswith("ci="):
                updated_cookies.append(f"ci={city_id}")
            elif part.startswith("recentCis="):
                updated_cookies.append(f"recentCis={recent_cis}")
            else:
                updated_cookies.append(part)
