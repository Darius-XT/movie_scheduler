...
class UpdateMovieItemScraper:
    ...
    def scrape_movie_list(
        self, show_type: int, page: int = 1, city: str | None = None
    ) -> tuple[bool, str]:
        try:
            if city is None:
                city = settings.city
            ...
            logger.debug(
                f"开始爬取电影列表: showType={show_type}, page={page}, offset={offset}, city={city}"
            )
...
update_movie_item_scraper = UpdateMovieItemScraper()
